...
class ml_fitting:
    ...
    def train_models(self, chosen_models=['Logistic', 'Naive_Bayes']):
        ...
        X_train : pd.DataFrame.
                  DataFrame without 'SMURF' column.

        y_train : pd.DataFrame.
                  DataFrame with only the 'SMURF' column.
    ...
    def compare_models(self, X_test, y_test, chosen_models=None):
        ...
        y_test: pd.DataFrame.
                DataFrame with only the 'SMURFS' column.
    ...
    def predict(self, X_test, chosen_models=None):
        """
        This function provides a final classification for a given sample. Each classification
        is a prediction made by a chosen model. 
        
        Parameters
        ----------
        X_smurf: pd.DataFrame
                 DataFrame containing samples to classify. 
        ...
        chosen_models: list.
                       ...
                       'Logistic', 'Naive_Bayes','Random_Forest', 'MLP', 'XGBosst' 
                       and 'SVC'.
        
        only_smf: bool, default True.
                  If True, a DataFrame with only positive classifications will be 
                  returned.
        ...
        for model in selected_trained_h_models.keys():
            print(f'{model}')
            preds[model] = selected_trained_h_models[model].predict(np.matrix(X_test[features]))
            preds[model] = preds[model].astype(int) 

        #         X_test = classify[selected_trained_h_models.keys()]
        #         predictions = pd.concat([df, X_test], axis=1)
...
#         avaiable_models = [m for m in models if m in self.choosen_models]
        ...
        if self.choosen_models == None:
            raise Exception(f'Select at least one trained model to make predicitons. The available models are: \n {avaiable_models}')
